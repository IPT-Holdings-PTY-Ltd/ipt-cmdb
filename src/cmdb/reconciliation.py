...
STRONG_IDENTIFIERS = ("device_uuid", "bios_uuid", "serial_number", "domain_sid", "license_key_hash")
...
def decide_ci_match(
    *, connection_id: str, external_type: str, external_id: str, identifiers: Mapping[str, str],
    mappings: Iterable[Mapping[str, str]], identifier_index: Mapping[tuple[str, str], str],
) -> MatchDecision:
    ...
    for mapping in mappings:
        if (mapping.get("connection_id"), mapping.get("external_type"), mapping.get("external_id")) == (connection_id, external_type, external_id):
            return MatchDecision("mapped", mapping["ci_id"], 1.0, "existing provider ID mapping")
    matches = {identifier_index[(kind, value)] for kind, value in identifiers.items() if kind in STRONG_IDENTIFIERS and value and (kind, value) in identifier_index}
    if len(matches) == 1:
...
def can_apply_field(provider: str, field: str, priorities: Mapping[tuple[str, str], int], current_provider: str | None) -> bool:
