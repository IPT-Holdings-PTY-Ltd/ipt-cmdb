...
SEED: dict[str, Any] = {
    "companies": [
        {
            ...
            "externalIds": {"connectwise": "125"},
        },
        {
            "id": "northwind",
            "name": "Northwind Traders",
            "externalIds": {"connectwise": "222"},
        },
    ],
    "users": [
        {
            ...
            "passwordHash": "pbkdf2_sha256$310000$cSTaQ5-dvwYYII0lPZEJFQ==$lyUPmkLv2PnaWrfhryHh19M_L8OWGRHzy2DtNzkVchc=",
            "role": "platform_admin",
            "companyIds": ["*"],
        },
        {
            ...
            "passwordHash": "pbkdf2_sha256$310000$wHqVsKnu-McTf-ZW-2l-gA==$wuFxNKtUAwz3F98GpQ-MAawD-ViHYN8Atyr8AZ_eqtQ=",
            "role": "msp_operator",
        ...
        },
        {
            ...
            "passwordHash": "pbkdf2_sha256$310000$NglhiTlhCfZqjuiUUVCggw==$GhspKU-GzdhnK1WbjWrQaCW-UAAoo_mQ7Znc4y4rWyo=",
            "role": "client_reader",
            "companyIds": ["acme"],
        },
    ],
    "assets": [
        ...
        },
        {
            ...
            "companyId": "acme",
            "name": "Alice Smith",
            "type": "Credential owner",
            "status": "Active",
            "source": "passportal",
            "externalId": "pp-445",
            "lastSeen": "2026-07-10T06:50:00Z",
            "fields": {"email": "alice@acme.example"},
        },
        {
            "id": "asset-3",
            "companyId": "northwind",
            ...
            "source": "ncentral",
            "externalId": "nc-1022",
            "lastSeen": "2026-07-09T19:00:00Z",
            "fields": {"os": "Windows 11 Pro", "serial": "NW022"},
        },
    ],
...
def postgres_connection(
    ...
    *,
    ...
    try:
        ...
        raise RuntimeError(
            "PostgreSQL mode needs `python -m pip install -r requirements.txt`."
        ) from error
...
def build_seed_state(mode: str, source: dict | None = None) -> dict:
    ...
    if mode == "demo":
        ...
        administrators = [
            item for item in source.get("users", []) if item.get("role") == "platform_admin"
        ]
        ...
        state = {
            ...
            "changes": [],
            "branding": {},
            "mspBranding": deepcopy(
                source.get("mspBranding")
                or {
                    "name": "CMDB Hub",
                    "accent": "#50d5b9",
            ...
            "integrations": [
                ...
                for item in source.get("integrations", SEED["integrations"])
            ],
            "syncRuns": [],
            "auditEvents": [],
        }
...
def save_database_settings(data: dict) -> dict:
    global \
        ...
        DATABASE_ERROR, \
        DATABASE_MODE, \
        CANONICAL_DATABASE_INITIALIZED
    if (
        os.getenv("DATABASE_URL")
        and os.getenv("ALLOW_UI_DATABASE_CONFIG", "false").lower() != "true"
    ):
        return {
            "ok": False,
    ...
    with postgres_connection(database_url) as connection, connection.cursor() as cursor:
        ...
        if legacy_table and not initialized:
            cursor.execute(
                "SELECT state FROM legacy_application_state WHERE state_key = 'cmdb_api'"
            )
    ...
    return {
        "ok": True,
        "message": "PostgreSQL configured and activated."
        if not seeded
        else f"PostgreSQL initialized with the {seed_mode} seed.",
        "seeded": seeded,
        "seedMode": seed_mode if seeded else "existing",
        **database_status(),
    }
        ...
        try:
            ...
            with postgres_connection() as connection, connection.cursor() as cursor:
                ...
                if cursor.fetchone()[0]:
                    cursor.execute(
                        "SELECT state FROM legacy_application_state WHERE state_key = %s",
                        ("cmdb_api",),
                    )
                    legacy_row = cursor.fetchone()
...
def save_db(value: dict) -> None:
    ...
    if DATABASE_MODE == "database setup":
        return
    DATA_DIR.mkdir(parents=True, exist_ok=True)
    DATA_FILE.write_text(json.dumps(value, indent=2))
...
def backup_document(state: dict | None = None) -> dict:
    ...
    return {
        ...
        "included": [
            "customers",
            "contacts and responsibility history",
            "users and local credential hashes",
            "configuration items",
            "relationships",
            "changes",
            "integrations",
...
def preview_backup(document: dict) -> dict:
    if document.get("format") != "cmdb-hub-backup" or document.get("version") not in {
        1,
    ...
    if not isinstance(state, dict) or any(
        not isinstance(state.get(key), list) for key in required_lists
    ):
        raise ValueError("The backup is missing required CMDB collections.")
    if not any(
        user.get("role") == "platform_admin" and user.get("id")
    ...
    if document.get("version") == PORTABLE_BACKUP_VERSION:
        ...
        if document.get("checksum") != expected:
            raise ValueError(
                "The portable backup checksum does not match; the file may be incomplete or modified."
            )
    return {
        "valid": True,
...
def restore_backup(document: dict) -> dict:
    ...
    if not isinstance(state, dict):
        raise ValueError("Portable backup state must be an object")
    state.setdefault("branding", {})
    state.setdefault(
        "mspBranding",
        {
    ...
    return {
        ...
        "restoredFrom": document.get("createdAt", "an unknown date"),
...
ROLE_TEMPLATES = [
    {
        ...
        "system": True,
        "permissions": [
            "Manage customers, groups and users",
            "Manage MSP integrations and branding",
            "Manage RBAC and database configuration",
            "Full customer CMDB access",
        ],
    },
    {
        "id": "msp_operator",
        "name": "MSP operator",
        "scope": "Assigned customer groups",
        "system": True,
        "permissions": [
            "View and manage assigned customer CIs",
            "Manage customer users",
            "Run approved integration syncs",
            "No RBAC or database configuration",
        ],
    },
    {
        "id": "client_reader",
        ...
        "system": True,
        "permissions": [
            "View permitted customer CIs and relationships",
...
def allowed(user: dict | None, company_id: str) -> bool:
    ...
    return bool(
        ...
        and (
            ...
            or company_id in user["companyIds"]
        )
    )
...
def can_manage(user: dict, company_id: str) -> bool:
    return bool(
        user and user["role"] in {"platform_admin", "msp_operator"} and allowed(user, company_id)
    )


def known_company(company_id: str | None) -> bool:
    return bool(company_id and any(company["id"] == company_id for company in DB["companies"]))
...
def relationship_exists(
    relationships: list[dict], from_id: str, to_id: str, relationship_type: str
) -> bool:
...
def asset_metadata(asset: dict) -> dict:
    ...
    if not asset.get("metadata") and asset.get("status") == "Retired":
        metadata["lifecycle"] = "retired"
    elif not asset.get("metadata") and asset.get("status") == "Planned":
        metadata["lifecycle"] = "planned"
    elif not asset.get("metadata") and asset.get("status") == "Active":
        metadata["operationalStatus"] = "healthy"
    return metadata
...
def normalise_metadata(value: dict | None, status: str | None = None) -> dict:
    ...
    metadata = {
        ...
        if key in METADATA_DEFAULTS and item is not None
    }
    if metadata["lifecycle"] not in LIFECYCLE_STATES:
        raise ValueError("Choose a valid lifecycle state")
    if metadata["operationalStatus"] not in OPERATIONAL_STATES:
        raise ValueError("Choose a valid operational status")
    if metadata["criticality"] not in CRITICALITY_LEVELS:
        raise ValueError("Choose a valid criticality")
    if metadata["environment"] not in ENVIRONMENTS:
        raise ValueError("Choose a valid environment")
    if metadata["dataClassification"] not in DATA_CLASSIFICATIONS:
        raise ValueError("Choose a valid data classification")
    if metadata["customerFacing"] not in YES_NO_VALUES:
        raise ValueError("Choose whether the service is customer-facing")
    if metadata["signoffRequired"] not in YES_NO_VALUES:
        raise ValueError("Choose whether business signoff is required")
    if metadata["haEnabled"] not in YES_NO_VALUES:
        raise ValueError("Choose whether high availability is enabled")
    if metadata["maintenanceMode"] not in YES_NO_VALUES:
        raise ValueError("Choose whether maintenance mode is enabled")
    if metadata["capacityStatus"] not in CAPACITY_STATES:
        raise ValueError("Choose a valid cluster capacity state")
    if metadata["mobility"] not in MOBILITY_STATES:
        raise ValueError("Choose a valid virtual machine mobility mode")
    if metadata["powerState"] not in POWER_STATES:
        raise ValueError("Choose a valid power state")
    if metadata["protectionStatus"] not in PROTECTION_STATES:
        raise ValueError("Choose a valid protection status")
    if metadata["displayLayer"] not in DISPLAY_LAYERS:
        raise ValueError("Choose a valid relationship display layer")
    if status == "Retired":
        metadata["lifecycle"] = "retired"
    return metadata
...
def asset_view(asset: dict) -> dict:
    return {**asset, "metadata": asset_metadata(asset)}
...
def attention_items(
    ...
    for asset in assets:
        ...
        if metadata["lifecycle"] in {"retired", "disposed"}:
            continue
        for kind, field in (
            ("Renewal", "renewalDate"),
            ("End of life", "endOfLifeDate"),
        ):
                ...
                items.append(
                    {
                        ...
                        "companyName": company_names.get(asset["companyId"], asset["companyId"]),
                        "kind": kind,
                        "date": value,
                        "days": days,
                        "criticality": metadata["criticality"],
                        "owner": metadata["technicalOwner"]
                        or metadata["serviceOwner"]
...
def customer_overview(companies: list[dict], assets: list[dict]) -> list[dict]:
    attention = attention_items(assets, companies)
    by_company: dict[str, list[dict]] = {company["id"]: [] for company in companies}
    for asset in assets:
        by_company.setdefault(asset["companyId"], []).append(asset)
    result = []
    for company in companies:
        company_assets = by_company.get(company["id"], [])
        items = [item for item in attention if item["companyId"] == company["id"]]
        result.append(
            {
                ...
                "criticalCount": sum(
                    asset_metadata(asset)["criticality"] == "critical" for asset in company_assets
                ),
                "attentionCount": len(items),
                "overdueCount": sum(item["days"] < 0 for item in items),
            }
        )
    return result
...
_DASHBOARD_IMPACT_RULES = {
    "connected_to": (False, True),
    "depends_on": (True, True),
    "installed_on": (True, True),
    "licensed_to": (False, True),
    "used_by": (False, True),
    "related_to": (False, False),
    "hosts": (False, True),
    "backs_up": (False, False),
    "managed_by": (True, True),
    "member_of": (False, False),
    "stored_on": (True, True),
    "provided_by": (True, True),
...
def dashboard_display_layer(asset: dict) -> str:
    metadata = asset_metadata(asset)
    explicit = metadata.get("displayLayer")
    if explicit in _DASHBOARD_LAYER_LABELS:
        return explicit
    asset_type = str(asset.get("type", "")).lower()
    signal = f"{asset_type} {asset.get('name', '')} {metadata.get('vendor', '')}".lower()
    if asset_type == "business system" or "credential" in asset_type or "person" in asset_type:
        return "business"
    if any(value in asset_type for value in ("software", "service", "licence", "license")):
        return (
            "storage"
            if any(value in signal for value in ("backup", "veeam", "repository"))
            else "application"
        )
    if asset_type in {"datastore", "storage array"}:
        return "storage"
    if "virtualization" in asset_type or asset_type in {
        "hypervisor host",
        "virtual network",
    }:
        return "virtualization"
    if (
        "network" in asset_type
        or "vpn" in asset_type
        or any(value in signal for value in ("firewall", "switch", "router", "wan", "internet"))
    ):
        return "foundation" if any(value in signal for value in ("wan", "internet")) else "network"
    if (
        asset_type == "virtual machine"
        or any(value in asset_type for value in ("server", "workstation"))
        or asset_type == "device"
    ):
        return "compute"
    return "foundation"
...
def _dashboard_owner(asset: dict) -> str:
    metadata = asset_metadata(asset)
    if asset.get("type") == "Business system" and metadata.get("businessOwner"):
        return metadata["businessOwner"]
    return (
...
def _dashboard_stale(asset: dict, today: date) -> bool:
    if asset.get("source") == "manual" or not asset.get("lastSeen"):
        return False
    try:
        seen = datetime.fromisoformat(str(asset["lastSeen"]).replace("Z", "+00:00")).date()
    except ValueError:
        return True
...
def _dashboard_business_scope(system_id: str, relationships: list[dict]) -> set[str]:
    ...
    for relationship in relationships:
        ...
        reverse_for_impact, traverses = _DASHBOARD_IMPACT_RULES.get(
            relationship_type if isinstance(relationship_type, str) else "",
            (False, False),
        )
        if not traverses or relationship.get("impactPolicy") == "informational":
            continue
        source, target = relationship.get("fromId"), relationship.get("toId")
        if not isinstance(source, str) or not isinstance(target, str):
            continue
        if reverse_for_impact:
...
def dashboard_snapshot(
    ...
    sync_runs = sync_runs or []
    company_names = {company["id"]: company["name"] for company in companies}
    visible_assets = [
        asset_view(asset)
        for asset in assets
        if not company_id or asset.get("companyId") == company_id
    ]
    asset_ids = {asset["id"] for asset in visible_assets}
    visible_relationships = [
        item
        for item in relationships
        if item.get("fromId") in asset_ids and item.get("toId") in asset_ids
    ]
    relationship_counts = {asset_id: 0 for asset_id in asset_ids}
    for relationship in visible_relationships:
        relationship_counts[relationship["fromId"]] += 1
        relationship_counts[relationship["toId"]] += 1
    lifecycle_by_asset: dict[str, list[dict]] = {}
    for item in attention_items(visible_assets, companies, today=today):
        lifecycle_by_asset.setdefault(item["assetId"], []).append(item)
    ...
    for asset in visible_assets:
        ...
        if operational in {"offline", "degraded"}:
            reasons.append(f"Operational status: {operational}")
            score += 6 if operational == "offline" else 4
        for item in lifecycle_by_asset.get(asset["id"], []):
            reasons.append(
                f"{item['kind']}: {abs(item['days'])} days overdue"
                if item["days"] < 0
                else f"{item['kind']}: {item['days']} days"
            )
            score += 5 if item["days"] < 0 else 3
        if not _dashboard_owner(asset):
            reasons.append("Accountable owner missing")
            score += 2
        if relationship_counts.get(asset["id"], 0) == 0:
            reasons.append("No relationships recorded")
            score += 1
        if _dashboard_stale(asset, today):
            reasons.append("Integration data is stale")
            score += 2
        if reasons:
            attention.append(
                {
                    ...
                    "companyId": asset["companyId"],
                    "companyName": company_names.get(asset["companyId"], asset["companyId"]),
                    "owner": _dashboard_owner(asset) or "Unassigned",
                    "reasons": reasons,
                    "score": score,
                    "severity": "critical" if score >= 6 else "warning" if score >= 3 else "info",
                }
            )
    ...
    for system in [asset for asset in visible_assets if asset.get("type") == "Business system"]:
        ...
        for asset_id in scope:
            memberships.setdefault(asset_id, set()).add(system["id"])
        ...
        business_systems.append(
            {
                "id": system["id"],
                "name": system["name"],
                "companyId": system["companyId"],
                "companyName": company_names.get(system["companyId"], system["companyId"]),
                "owner": metadata.get("businessOwner") or "Unassigned",
                "serviceOwner": metadata.get("serviceOwner") or "",
                "operationalStatus": metadata.get("operationalStatus") or "unknown",
                "criticality": metadata.get("criticality") or "medium",
                "rtoHours": metadata.get("rtoHours") or "",
                "rpoHours": metadata.get("rpoHours") or "",
                "supportCount": len(support),
                "attentionCount": len(support_attention),
            }
        )
    ...
    for layer, label in _DASHBOARD_LAYER_LABELS.items():
        layer_assets = [
            asset for asset in visible_assets if dashboard_display_layer(asset) == layer
        ]
        layer_coverage.append(
            {
                "id": layer,
                "label": label,
                "count": len(layer_assets),
                "attentionCount": sum(
                    any(item["assetId"] == asset["id"] for item in attention)
    ...
    for company in companies:
        ...
        company_lifecycle = [
            item
            for items in lifecycle_by_asset.values()
            for item in items
        ...
        customer_rows.append(
            {
                ...
                "companyName": company["name"],
                "assetCount": len(company_assets),
                "businessSystemCount": sum(
                    asset["type"] == "Business system" for asset in company_assets
                ),
                "unhealthyCount": sum(
                    asset["metadata"].get("operationalStatus") in {"offline", "degraded"}
                    for asset in company_assets
                ),
                "attentionCount": len(company_attention),
                "overdueCount": sum(item["days"] < 0 for item in company_lifecycle),
                "missingOwnerCount": sum(not _dashboard_owner(asset) for asset in company_assets),
                "staleCount": sum(_dashboard_stale(asset, today) for asset in company_assets),
                "riskScore": sum(item["score"] for item in company_attention),
            }
        )
    ...
    for run in sorted(
        sync_runs,
        key=lambda item: item.get("startedAt") or item.get("finishedAt") or "",
        reverse=True,
    ):
        latest_runs.setdefault(run.get("type", ""), run)
    integration_rows = []
    for item in integrations:
        configured_in_environment = configured(item.get("type", ""))
        status = (
            "Ready"
            if configured_in_environment and item.get("status") == "Not configured"
        ...
        integration_rows.append(
            {
                **item,
                "enabled": configured_in_environment or item.get("enabled", False),
                "status": status,
                "lastRunStatus": last_run.get("status") if last_run else "never",
                "lastRunAt": (last_run.get("finishedAt") or last_run.get("startedAt"))
                if last_run
                else item.get("lastSync"),
            }
        )

    upcoming_changes = []
    for change in changes:
        if company_id and change.get("companyId") != company_id:
            continue
        planned = change.get("plannedStart")
        if not planned:
            continue
        try:
            planned_date = datetime.fromisoformat(str(planned).replace("Z", "+00:00")).date()
        except ValueError:
            continue
        if planned_date < today or str(change.get("status", "")).lower() in {
            "completed",
            "cancelled",
        }:
            continue
        upcoming_changes.append(
            {
                ...
                "companyName": company_names.get(change.get("companyId"), change.get("companyId")),
                "plannedStart": planned,
                "riskLevel": change.get("riskLevel", "medium"),
                "impactCount": len(change.get("impactSnapshot") or []),
            }
        )
    ...
    return {
        ...
        "summary": {
            "customers": len({asset["companyId"] for asset in visible_assets})
            if company_id
            else len(companies),
            "assets": len(visible_assets),
            "businessSystems": len(business_systems),
            "unhealthy": unhealthy,
            "attention": len(attention),
            "missingOwners": missing_owner,
            "unlinked": unlinked,
            "stale": stale,
            "sharedDependencies": sum(len(system_ids) > 1 for system_ids in memberships.values()),
            "integrationIssues": sum(
                str(item.get("status", "")).lower()
                not in {"ready", "connected", "success", "healthy"}
                or item.get("lastRunStatus") in {"failed", "blocked"}
        ...
        "upcomingChanges": upcoming_changes[:8],
    }
...
def configured(kind: str) -> bool:
    if kind == "connectwise":
        return all(
            os.getenv(x)
            for x in ("CW_BASE_URL", "CW_COMPANY_ID", "CW_PUBLIC_KEY", "CW_PRIVATE_KEY")
        )
    key = kind.upper()
    return bool(os.getenv(f"{key}_BASE_URL") and os.getenv(f"{key}_API_TOKEN"))
...
DEMO_ASSETS: list[dict[str, Any]] = [
    {
        ...
        "source": "demo",
        "externalId": None,
        "lastSeen": "2026-07-10T08:00:00Z",
        "fields": {"role": "Edge firewall"},
    },
    {
        "id": "demo-core",
        "companyId": "acme",
        ...
        "externalId": None,
        "lastSeen": "2026-07-10T08:00:00Z",
        "fields": {"role": "Core switch"},
    },
    {
        "id": "demo-access",
        "companyId": "acme",
        "name": "ACME-ACCESS-SW01",
        "type": "Network device",
        "status": "Active",
        "source": "demo",
        "externalId": None,
        "lastSeen": "2026-07-10T08:00:00Z",
        "fields": {"role": "Access switch"},
    },
    {
        "id": "demo-db",
        "companyId": "acme",
        ...
        "externalId": None,
        "lastSeen": "2026-07-10T08:00:00Z",
        "fields": {"role": "Database server"},
        "metadata": {
            "virtualizationPlatform": "VMware vSphere",
            "clusterName": "ACME-PROD-CL01",
            "haEnabled": "yes",
            "mobility": "automatic",
            "powerState": "running",
    ...
    },
    {
        ...
        "externalId": None,
        "lastSeen": "2026-07-10T08:00:00Z",
        "fields": {"role": "Application server"},
        "metadata": {
            "virtualizationPlatform": "VMware vSphere",
            "clusterName": "ACME-PROD-CL01",
            "haEnabled": "yes",
    ...
    },
    {
        ...
        "externalId": None,
        "lastSeen": "2026-07-10T08:00:00Z",
        "fields": {"service": "Customer portal"},
    },
    {
        "id": "demo-dbapp",
        "companyId": "acme",
        ...
        "externalId": None,
        "lastSeen": "2026-07-10T08:00:00Z",
        "fields": {"database": "finance"},
    },
    {
        "id": "demo-license",
        "companyId": "acme",
        "name": "PostgreSQL Enterprise Licence",
        "type": "Licence",
        "status": "Active",
        "source": "demo",
        "externalId": None,
        "lastSeen": "2026-07-10T08:00:00Z",
        "fields": {"renewal": "2027-06-30"},
    },
    {
        "id": "demo-backup",
        "companyId": "acme",
        "name": "Veeam Backup Agent",
        "type": "Software",
        "status": "Active",
        "source": "demo",
        "externalId": None,
        "lastSeen": "2026-07-10T08:00:00Z",
        "fields": {"policy": "Nightly"},
    },
    {
        "id": "demo-cluster",
        "companyId": "acme",
        ...
        "externalId": None,
        "lastSeen": "2026-07-10T08:00:00Z",
        "fields": {"role": "Production compute cluster"},
        "metadata": {
            "virtualizationPlatform": "VMware vSphere",
            "haEnabled": "yes",
            "minimumHosts": "1",
    ...
    },
    {
        ...
        "externalId": None,
        "lastSeen": "2026-07-10T08:00:00Z",
        "fields": {"role": "Current host for finance workloads"},
        "metadata": {
            "vendor": "VMware",
            "model": "ESXi 8",
            "virtualizationPlatform": "VMware vSphere",
    ...
    },
    {
        ...
        "externalId": None,
        "lastSeen": "2026-07-10T08:00:00Z",
        "fields": {"capacity": "8 TB", "free": "3.2 TB"},
        "metadata": {
            "vendor": "VMware",
            "virtualizationPlatform": "VMware vSphere",
            "technicalOwner": "Platform Team",
    ...
    },
    {
        ...
        "externalId": None,
        "lastSeen": "2026-07-10T08:00:00Z",
        "fields": {"role": "Shared production storage"},
        "metadata": {
            "vendor": "Dell",
            "model": "PowerVault",
            "technicalOwner": "Platform Team",
    ...
    },
    {
        ...
        "externalId": None,
        "lastSeen": "2026-07-10T08:00:00Z",
        "fields": {"service": "Finance and accounting"},
        "metadata": {
            "lifecycle": "in_service",
            "operationalStatus": "healthy",
            "criticality": "critical",
            ...
            "serviceOwner": "Finance Applications",
            "technicalOwner": "Managed Services",
            "signoffDelegate": "Financial Controller",
            "department": "Finance",
            "userPopulation": "42 finance users",
            "rtoHours": "4",
            "rpoHours": "1",
            "supportHours": "Business hours + month end",
            "dataClassification": "confidential",
            "customerFacing": "no",
            "signoffRequired": "yes",
            "aliases": "Sage, Finance ERP",
            "businessDescription": "Finance, purchasing, stock and management reporting.",
        },
    },
    {
        "id": "demo-reporting",
        ...
        "externalId": None,
        "lastSeen": "2026-07-10T08:00:00Z",
        "fields": {"service": "Management reporting"},
        "metadata": {
            "lifecycle": "in_service",
            "operationalStatus": "healthy",
            "criticality": "high",
            ...
            "serviceOwner": "Business Intelligence",
            "technicalOwner": "Managed Services",
            "signoffDelegate": "Finance Director",
            "department": "Executive",
            "userPopulation": "12 decision makers",
            "rtoHours": "8",
            "rpoHours": "4",
            ...
            "signoffRequired": "yes",
            "aliases": "BI Reports",
            "businessDescription": "Executive dashboards and scheduled management packs.",
        },
    },
    {
        "id": "demo-wan",
        "companyId": "acme",
        "name": "Internet / MPLS WAN",
        "type": "Network service",
        "status": "Active",
        "source": "demo",
        "externalId": None,
        "lastSeen": "2026-07-10T08:00:00Z",
        "fields": {"role": "Upstream connectivity"},
        "metadata": {
            "displayLayer": "foundation",
            "networkZone": "WAN",
            "networkRole": "internet_wan",
...
DEMO_NETWORK_METADATA: dict[str, dict[str, str]] = {
    "demo-fw": {
        "displayLayer": "network",
        "site": "Head Office",
        "networkZone": "Edge",
        "ipAddress": "203.0.113.10",
        "networkRole": "firewall",
        "redundancyGroup": "ACME-EDGE",
    ...
    "demo-access": {
        ...
        "networkRole": "access_switch",
        "vlanId": "110",
        "subnet": "10.10.110.0/24",
        "vendor": "Cisco",
    },
    "demo-ws": {
        "displayLayer": "compute",
        "site": "Head Office",
        "networkZone": "Finance",
        "vlanId": "110",
        "subnet": "10.10.110.0/24",
        "ipAddress": "10.10.110.21",
    },
    "demo-printer": {
        "displayLayer": "network",
        "site": "Head Office",
        ...
        "vlanId": "110",
        "subnet": "10.10.110.0/24",
        "ipAddress": "10.10.110.40",
        "networkRole": "endpoint",
    },
    "demo-crm": {"displayLayer": "application"},
    "demo-dbapp": {"displayLayer": "application"},
    "demo-license": {"displayLayer": "application"},
    "demo-backup": {"displayLayer": "storage"},
    "demo-db": {"displayLayer": "compute"},
    "demo-app": {"displayLayer": "compute"},
    "demo-files": {"displayLayer": "compute"},
    "demo-cluster": {"displayLayer": "virtualization"},
    "demo-esx1": {"displayLayer": "virtualization"},
    "demo-esx2": {"displayLayer": "virtualization"},
    "demo-ds1": {"displayLayer": "storage"},
    "demo-san": {"displayLayer": "storage"},
    "demo-vcenter": {"displayLayer": "virtualization"},
    "demo-sage200": {"displayLayer": "business"},
    "demo-reporting": {"displayLayer": "business"},
}
for _demo_asset in DEMO_ASSETS:
    if _demo_asset["id"] in DEMO_NETWORK_METADATA:
        _demo_asset["metadata"] = {
            **(_demo_asset.get("metadata") or {}),
            **DEMO_NETWORK_METADATA[_demo_asset["id"]],
        }
DEMO_RELATIONSHIPS = [
    {"id": "demo-r1", "fromId": "demo-fw", "toId": "demo-core", "type": "connected_to"},
    {
        "id": "demo-r2",
        "fromId": "demo-core",
        "toId": "demo-access",
    ...
    {"id": "demo-r3", "fromId": "demo-core", "toId": "demo-db", "type": "connected_to"},
    {
        "id": "demo-r4",
        "fromId": "demo-core",
        "toId": "demo-app",
    ...
    {"id": "demo-r8", "fromId": "demo-crm", "toId": "demo-app", "type": "installed_on"},
    {
        "id": "demo-r9",
        "fromId": "demo-dbapp",
        "toId": "demo-db",
    ...
    {"id": "demo-r12", "fromId": "demo-app", "toId": "demo-db", "type": "depends_on"},
    {"id": "demo-r13", "fromId": "demo-ws", "toId": "demo-crm", "type": "depends_on"},
    {
        "id": "demo-r14",
        "fromId": "demo-printer",
        "toId": "demo-files",
...
def add_demo_data() -> dict:
    asset_ids, relation_ids = (
        {item["id"] for item in DB["assets"]},
        {item["id"] for item in DB["relationships"]},
    )
    assets, relations = (
        [item for item in DEMO_ASSETS if item["id"] not in asset_ids],
        [item for item in DEMO_RELATIONSHIPS if item["id"] not in relation_ids],
    )
    DB["assets"].extend(assets)
    DB["relationships"].extend(relations)
    save_db(DB)
...
def execute_sync(kind: str) -> dict:
    ...
    if not configured(kind):
        run.update(
            status="blocked",
            message="Integration variables are not configured. See .env.example.",
        )
        ...
        try:
            ...
            with urlopen(  # nosec B310
                ...
                run["discovered"] = len(json.loads(response.read()))
                run["message"] = "Connection verified. Company import is deliberately review-gated."
        except HTTPError as error:
            run.update(status="failed", message=f"ConnectWise returned {error.code}")
        except URLError as error:
            run.update(
                status="failed",
                message=f"ConnectWise connection failed: {error.reason}",
            )
    else:
        run["message"] = (
            f"{kind} adapter configuration detected; endpoint mapping remains review-gated."
        )
    run["finishedAt"] = now()
    return run
    ...
    with LOCK:
        ...
        if integration:
            integration.update(
                ...
                status="Healthy" if run["status"] == "success" else run["status"],
                enabled=configured(kind),
            )
        save_db(DB)
    return run
