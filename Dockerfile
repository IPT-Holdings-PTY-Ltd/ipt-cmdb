...
ENV PORT=3000 DATA_DIR=/app/data FORWARDED_ALLOW_IPS= PYTHONDONTWRITEBYTECODE=1 PYTHONUNBUFFERED=1
USER cmdb
EXPOSE 3000
HEALTHCHECK --interval=30s --timeout=5s --start-period=10s --retries=3 \
  CMD python -c "import urllib.request; urllib.request.urlopen('http://localhost:3000/api/live', timeout=3)"
CMD ["python", "-m", "uvicorn", "backend.main:app", "--host", "0.0.0.0", "--port", "3000", "--proxy-headers"]
