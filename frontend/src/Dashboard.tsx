...
function MetricCard({ label, value, detail, color, icon }: { label: string; value: number; detail: string; color: string; icon: ReactNode }) {
  return <Paper className="dashboard-metric" sx={{ '--metric-color': color }}>
    <Stack direction="row" justifyContent="space-between" alignItems="flex-start"><Box><Typography variant="caption" color="text.secondary">{label}</Typography><Typography variant="h3">{value}</Typography></Box><Box className="dashboard-metric-icon">{icon}</Box></Stack>
    <Typography variant="caption" color="text.secondary">{detail}</Typography>
  </Paper>;
}

function SectionHeading({ eyebrow, title, copy, action }: { eyebrow: string; title: string; copy?: string; action?: ReactNode }) {
  return <Stack direction={{ xs: 'column', sm: 'row' }} justifyContent="space-between" alignItems={{ sm: 'flex-start' }} spacing={1.5} sx={{ mb: 2 }}>
    <Box><Typography variant="overline" color="primary">{eyebrow}</Typography><Typography variant="h5">{title}</Typography>{copy && <Typography variant="body2" color="text.secondary">{copy}</Typography>}</Box>{action}
  </Stack>;
}
...
function AttentionQueue({ items, showCustomer = false }: { items: DashboardAttention[]; showCustomer?: boolean }) {
  ...
  return <Stack divider={<Box className="dashboard-divider" />}>
    {items.slice(0, 10).map(item => <Stack key={item.assetId} className="dashboard-attention-row" direction={{ xs: 'column', sm: 'row' }} justifyContent="space-between" alignItems={{ sm: 'center' }} spacing={1.5}>
      <Box sx={{ minWidth: 0 }}><Stack direction="row" spacing={1} alignItems="center" flexWrap="wrap" useFlexGap><Typography fontWeight={800}>{item.assetName}</Typography>{showCustomer && <Chip size="small" variant="outlined" label={item.companyName} />}</Stack><Typography variant="caption" color="text.secondary">{item.assetType} · {item.owner}</Typography><Typography variant="body2" color={item.severity === 'critical' ? 'error.light' : 'warning.light'}>{item.reasons.slice(0, 2).join(' · ')}</Typography></Box>
      <Stack direction="row" spacing={1} alignItems="center"><Chip size="small" color={item.severity === 'critical' ? 'error' : item.severity === 'warning' ? 'warning' : 'info'} label={`Score ${item.score}`} /><Button size="small" endIcon={<ArrowForwardOutlined />} onClick={() => navigate(`/assets/${item.assetId}/show`)}>Open</Button></Stack>
    </Stack>)}
  </Stack>;
}
...
function UpcomingChanges({ items, showCustomer = false }: { items: UpcomingChange[]; showCustomer?: boolean }) {
  const navigate = useNavigate();
  return <Card><CardContent><SectionHeading eyebrow="Change enablement" title="Upcoming changes" copy="Planned changes with saved CMDB impact snapshots." action={<Button size="small" onClick={() => navigate('/changes')}>Open change control</Button>} />
    {!items.length ? <Alert severity="info">No future change packages have a planned start date.</Alert> : <Stack spacing={1.25}>{items.map(item => <Paper variant="outlined" key={item.id} className="dashboard-change-row"><Stack direction="row" justifyContent="space-between" spacing={1}><Box><Typography fontWeight={800}>{item.title}</Typography><Typography variant="caption" color="text.secondary">{item.number}{showCustomer ? ` · ${item.companyName}` : ''}</Typography></Box><Chip size="small" color={riskColor(item.riskLevel)} label={sentence(item.riskLevel)} /></Stack><Stack direction="row" spacing={1} alignItems="center" sx={{ mt: 1 }}><ScheduleOutlined fontSize="small" color="action" /><Typography variant="body2">{dateTime(item.plannedStart)}</Typography><Typography variant="caption" color="text.secondary">· {item.impactCount} impacted CIs</Typography></Stack></Paper>)}</Stack>}
  </CardContent></Card>;
}
  ...
  return <>
    ...
    <Grid container spacing={2.5} sx={{ mt: 0.5 }}>
      ...
      <Grid size={{ xs: 12, xl: 4 }}><Card sx={{ height: '100%' }}><CardContent><SectionHeading eyebrow="Data sources" title="Integration health" copy="MSP-level discovery and reconciliation services." action={<Button size="small" onClick={() => navigate('/admin/integrations')}>Manage</Button>} />
        <Stack spacing={1.25}>{data.integrations.map(item => { const healthy = ['ready', 'connected', 'success', 'healthy'].includes(item.status.toLowerCase()) && !['failed', 'blocked'].includes(item.lastRunStatus); return <Paper variant="outlined" className="dashboard-integration-row" key={item.id}><Stack direction="row" justifyContent="space-between" spacing={1}><Box><Typography fontWeight={800}>{item.name}</Typography><Typography variant="caption" color="text.secondary">Last run: {dateTime(item.lastRunAt)}</Typography></Box><Chip size="small" color={healthy ? 'success' : 'warning'} label={item.status} /></Stack><Typography variant="caption" color={item.lastRunStatus === 'failed' || item.lastRunStatus === 'blocked' ? 'error.light' : 'text.secondary'}>Run status: {sentence(item.lastRunStatus)}</Typography></Paper>; })}{!data.integrations.length && <Alert severity="info">No MSP integrations are registered.</Alert>}</Stack>
      </CardContent></Card></Grid>
...
function CustomerDashboard({ data }: { data: DashboardData }) {
  ...
  return <>
    ...
    <Card sx={{ mt: 2.5 }}><CardContent><SectionHeading eyebrow="Business context" title="Business-system health" copy="The technical estate translated into the services and owners the customer recognises." action={<Button size="small" onClick={() => navigate('/business-systems')}>Manage systems</Button>} />
      {!data.businessSystems.length ? <Alert severity="info">No business systems have been modelled for this customer.</Alert> : <Grid container spacing={1.5}>{data.businessSystems.map(system => <Grid key={system.id} size={{ xs: 12, md: 6, xl: 4 }}><Paper variant="outlined" className="dashboard-system-card"><Stack direction="row" justifyContent="space-between" spacing={1}><Box sx={{ minWidth: 0 }}><Typography variant="h6" noWrap>{system.name}</Typography><Typography variant="caption" color="text.secondary">{system.owner} · {system.serviceOwner || 'No service owner'}</Typography></Box><Chip size="small" color={system.operationalStatus === 'healthy' ? 'success' : system.operationalStatus === 'offline' ? 'error' : 'warning'} label={sentence(system.operationalStatus)} /></Stack><Stack direction="row" spacing={1} flexWrap="wrap" useFlexGap sx={{ my: 1.5 }}><Chip size="small" variant="outlined" label={`${system.supportCount} supporting CIs`} /><Chip size="small" color={system.attentionCount ? 'warning' : 'success'} variant="outlined" label={`${system.attentionCount} attention`} /><Chip size="small" variant="outlined" label={`RTO ${system.rtoHours || '?'}h · RPO ${system.rpoHours || '?'}h`} /></Stack><Button size="small" endIcon={<ArrowForwardOutlined />} onClick={() => navigate(`/relationships?view=stack&businessAppId=${encodeURIComponent(system.id)}&sharedImpact=1`)}>View full stack</Button></Paper></Grid>)}</Grid>}
    </CardContent></Card>
    <Grid container spacing={2.5} sx={{ mt: 0.5 }}>
      ...
      <Grid size={{ xs: 12, xl: 5 }}><Card sx={{ height: '100%' }}><CardContent><SectionHeading eyebrow="Topology coverage" title="CMDB layers" copy="Coverage and attention items across the full service stack." />
        <Stack spacing={1.6}>{data.layers.map(layer => <Box key={layer.id}><Stack direction="row" justifyContent="space-between" spacing={1}><Typography variant="body2" fontWeight={750}>{layer.label}</Typography><Typography variant="caption" color="text.secondary">{layer.count} CIs · {layer.attentionCount} attention</Typography></Stack><LinearProgress variant="determinate" value={(layer.count / maxLayer) * 100} color={layer.attentionCount ? 'warning' : 'primary'} sx={{ mt: .6, height: 7, borderRadius: 6 }} /></Box>)}</Stack>
        <Stack direction="row" spacing={1} flexWrap="wrap" useFlexGap sx={{ mt: 2.5 }}><Chip color={data.summary.missingOwners ? 'warning' : 'success'} label={`${data.summary.missingOwners} missing owners`} /><Chip color={data.summary.unlinked ? 'warning' : 'success'} label={`${data.summary.unlinked} unlinked CIs`} /><Chip color={data.summary.stale ? 'warning' : 'success'} label={`${data.summary.stale} stale records`} /></Stack>
...
export function Dashboard() {
  ...
  return <Box className="dashboard-page">
    ...
    <Stack direction={{ xs: 'column', md: 'row' }} justifyContent="space-between" alignItems={{ md: 'flex-end' }} spacing={2} sx={{ mb: 3 }}><Box><Typography variant="overline" color="primary">{workspace.isRoot ? 'Managed service operations' : 'Customer service intelligence'}</Typography><Typography variant="h3">{workspace.isRoot ? 'MSP operational overview' : workspace.companyName}</Typography><Typography color="text.secondary" sx={{ mt: .5 }}>{workspace.isRoot ? 'Customer risk, data-source health and priority work across the managed estate.' : 'Business systems, supporting technology and the actions that protect service continuity.'}</Typography></Box>{data && <Chip variant="outlined" color={data.summary.attention ? 'warning' : 'success'} label={data.summary.attention ? `${data.summary.attention} items need review` : 'Estate healthy'} />}</Stack>
    {error && <Alert severity="error">{error}</Alert>}
    {!data && !error && <Paper className="dashboard-loading"><CircularProgress size={28} /><Typography color="text.secondary">Building operational overview…</Typography></Paper>}
