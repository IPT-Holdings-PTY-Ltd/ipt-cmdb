...
function PageHeading({ eyebrow, title, copy, action }: { eyebrow: string; title: string; copy: string; action?: React.ReactNode }) {
  return <Box sx={{ ...titleSx, display: 'flex', gap: 2, justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap' }}>
    <Box><Typography variant="overline" color="primary">{eyebrow}</Typography><Typography variant="h3">{title}</Typography><Typography color="text.secondary" sx={{ mt: 0.75, maxWidth: 780 }}>{copy}</Typography></Box>
    {action}
  </Box>;
}

function LoadingCard() {
  return <Card><CardContent><Stack direction="row" spacing={2} alignItems="center"><CircularProgress size={22} /><Typography>Loading configuration…</Typography></Stack></CardContent></Card>;
}
...
export function CustomersPage() {
  ...
  return <RootGuard adminOnly><Title title="Customers" /><PageHeading eyebrow="Organisation" title="Customer management" copy="Create and open isolated customer workspaces. Customer deletion is intentionally not offered while CIs, users, or source mappings may still depend on the tenant." />
    <Grid container spacing={3}>
      <Grid size={{ xs: 12, lg: 5 }}><Card><CardContent><Stack component="form" spacing={2} onSubmit={createCustomer}>
        <Box><Typography variant="h5">Add customer</Typography><Typography color="text.secondary">A stable customer ID is generated from the name when left blank.</Typography></Box>
        <TextField label="Customer name" value={name} onChange={event => setName(event.target.value)} required placeholder="Contoso Ltd" />
        <TextField label="Customer ID (optional)" value={slug} onChange={event => setSlug(event.target.value)} helperText="Lowercase letters, numbers and hyphens" inputProps={{ pattern: '[a-z0-9-]+' }} placeholder="contoso" />
      ...
      <Grid size={{ xs: 12, lg: 7 }}><Card><CardContent><Typography variant="h5">Managed customers</Typography><Typography color="text.secondary" sx={{ mb: 2 }}>{workspace.companies.length} customer workspaces</Typography>
        <Stack divider={<Divider flexItem />}>
          {workspace.companies.map(company => <Stack key={company.id} direction="row" alignItems="center" justifyContent="space-between" spacing={2} sx={{ py: 1.5 }}>
            <Box><Typography fontWeight={750}>{company.name}</Typography><Typography variant="caption" color="text.secondary">Tenant ID: {company.id}</Typography></Box>
...
export function UsersPage() {
  ...
  return <RootGuard><Title title="Users" /><PageHeading eyebrow="Access" title="User management" copy="Root users receive explicit customer scope from a reusable access group plus optional direct customer access. Customer users remain bound to one tenant." />
    ...
    <Grid container spacing={3}>
      <Grid size={{ xs: 12, xl: 5 }}><Card><CardContent><Stack component="form" spacing={2} onSubmit={createUser}>
        ...
        <TextField label="Temporary password" type="password" value={password} onChange={event => setPassword(event.target.value)} inputProps={{ minLength: 8 }} helperText="At least 8 characters; local authentication only" required />
        {accountType === 'root' ? <>
      ...
      <Grid size={{ xs: 12, xl: 7 }}><Card><CardContent><Typography variant="h5">Users in your scope</Typography><Typography color="text.secondary" sx={{ mb: 2 }}>Server-side permissions determine which records are visible here.</Typography>
...
export function CustomerGroupsPage() {
  ...
  return <RootGuard adminOnly><Title title="Customer groups" /><PageHeading eyebrow="Organisation" title="Customer groups" copy="Bundle customer scopes into reusable access groups for MSP users. The dynamic All managed customers group follows the tenant list automatically." />
    {notice && <Alert severity={notice.severity} sx={{ mb: 2 }}>{notice.message}</Alert>}
    <Grid container spacing={3}>
      ...
      <Grid size={{ xs: 12, lg: 7 }}><Stack spacing={2}>{groups.map(group => <Card key={group.id}><CardContent><Stack direction="row" justifyContent="space-between" spacing={2}><Box><Stack direction="row" alignItems="center" spacing={1}><Typography variant="h6">{group.name}</Typography>{group.system && <Chip size="small" label="Dynamic" color="primary" variant="outlined" />}</Stack><Typography color="text.secondary">{group.companyIds.includes('*') ? 'Every managed customer' : group.companyIds.map(companyName).join(', ')}</Typography></Box>{!group.system && <Stack direction="row"><Button size="small" startIcon={<EditOutlined />} onClick={() => edit(group)}>Edit</Button><Button size="small" color="error" startIcon={<DeleteOutline />} onClick={() => remove(group)}>Delete</Button></Stack>}</Stack></CardContent></Card>)}</Stack></Grid>
    </Grid>
  </RootGuard>;
}
...
export function RbacPage() {
  ...
  return <RootGuard adminOnly><Title title="Access control" /><PageHeading eyebrow="Access" title="Role-based access control" copy="Review the fixed role templates and preview the effective customer scope for any user. Custom roles are a future extension point; server permissions remain authoritative." />
    {error ? <ErrorNotice error={error} /> : null}
    <Grid container spacing={2} sx={{ mb: 3 }}>{roles.map(role => <Grid key={role.id} size={{ xs: 12, lg: 4 }}><Card sx={{ height: '100%' }}><CardContent><SecurityOutlined color="primary" /><Typography variant="h6" sx={{ mt: 1 }}>{role.name}</Typography><Chip size="small" label={role.scope} sx={{ my: 1 }} /><List dense>{role.permissions.map(permission => <ListItem key={permission} disableGutters><ListItemIcon sx={{ minWidth: 30 }}><CheckCircleOutline color="success" fontSize="small" /></ListItemIcon><ListItemText primary={permission} /></ListItem>)}</List></CardContent></Card></Grid>)}</Grid>
    <Card><CardContent><Typography variant="h5">Effective access preview</Typography><Typography color="text.secondary" sx={{ mb: 2 }}>Select a user to see the final role and tenant boundary resolved by the API.</Typography><FormControl fullWidth sx={{ maxWidth: 520, mb: 2 }}><InputLabel>User</InputLabel><Select label="User" value={selected} onChange={event => setSelected(event.target.value)}>{users.map(user => <MenuItem key={user.id} value={user.id}>{user.email} — {user.role.replaceAll('_', ' ')}</MenuItem>)}</Select></FormControl>{effective && <Alert severity="info"><Typography fontWeight={750}>{effective.user.email} · {effective.role.name}</Typography><Typography variant="body2">Customer scope: {effective.scope}</Typography></Alert>}</CardContent></Card>
  </RootGuard>;
}
...
export function IntegrationsPage() {
  ...
  return <RootGuard><Title title="Integrations" /><PageHeading eyebrow="Operations" title="Third-party integrations" copy="MSP-wide source adapters live here. Connection settings are supplied securely by the API host; discovery and import remain review-gated while mapping rules are being built." action={<Chip label="MSP scope" color="primary" variant="outlined" />} />
    {error ? <Box sx={{ mb: 2 }}><ErrorNotice error={error} /></Box> : null}
    <Grid container spacing={2}>{integrations.map(integration => <Grid key={integration.id} size={{ xs: 12, md: 4 }}><Card sx={{ height: '100%' }}><CardContent><Stack direction="row" justifyContent="space-between"><CloudSyncOutlined color="primary" /><Chip size="small" color={integration.enabled ? 'success' : 'default'} label={integration.status} /></Stack><Typography variant="h5" sx={{ mt: 2 }}>{integration.name}</Typography><Typography color="text.secondary" sx={{ my: 1 }}>{integration.enabled ? 'Host configuration detected' : 'Awaiting secure host configuration'}</Typography><Typography variant="caption" display="block">{integration.lastSync ? `Last checked ${new Date(integration.lastSync).toLocaleString()}` : 'No sync attempts yet'}</Typography><Button sx={{ mt: 2 }} fullWidth variant="outlined" disabled={busy === integration.type} onClick={() => sync(integration.type)}>{busy === integration.type ? 'Checking…' : 'Test & sync'}</Button></CardContent></Card></Grid>)}</Grid>
    <Card sx={{ mt: 3 }}><CardContent><Typography variant="h5">Recent sync activity</Typography><Typography color="text.secondary" sx={{ mb: 2 }}>Adapter checks and review-gated discovery results.</Typography>{!runs.length ? <Alert severity="info">No sync attempts yet.</Alert> : <TableContainer><Table size="small"><TableHead><TableRow><TableCell>Source</TableCell><TableCell>Status</TableCell><TableCell>Result</TableCell><TableCell>Finished</TableCell></TableRow></TableHead><TableBody>{runs.slice(0, 10).map(run => <TableRow key={run.id}><TableCell>{run.type}</TableCell><TableCell><Chip size="small" label={run.status} color={run.status === 'success' ? 'success' : run.status === 'failed' ? 'error' : 'warning'} /></TableCell><TableCell>{run.message}</TableCell><TableCell>{run.finishedAt ? new Date(run.finishedAt).toLocaleString() : 'In progress'}</TableCell></TableRow>)}</TableBody></Table></TableContainer>}</CardContent></Card>
  </RootGuard>;
}
...
export function BrandingPage() {
  ...
  return <RootGuard><Title title="Branding" /><PageHeading eyebrow="Settings" title="MSP branding" copy="Publish one trusted MSP identity across sign-in, workspace screens and generated customer reports." action={<Chip label={brand.logoDataUrl ? 'Logo configured' : 'Fallback mark active'} color={brand.logoDataUrl ? 'success' : 'default'} variant="outlined" />} />
    {notice && <Alert severity={notice.severity} sx={{ mb: 2 }}>{notice.message}</Alert>}
    <Grid container spacing={3}><Grid size={{ xs: 12, xl: 7 }}><Card><CardContent><Stack component="form" spacing={3} onSubmit={save}>
      <Box><Typography variant="h5">Brand identity</Typography><Typography color="text.secondary">Use a horizontal or compact transparent PNG where possible. The fallback letters remain available if the image cannot load.</Typography></Box>
      <Grid container spacing={2}><Grid size={{ xs: 12, md: 8 }}><TextField fullWidth label="MSP display name" value={brand.name} onChange={event => update('name', event.target.value)} required /></Grid><Grid size={{ xs: 12, md: 4 }}><TextField fullWidth label="Fallback letters" value={brand.logoText} onChange={event => update('logoText', event.target.value.toUpperCase().slice(0, 3))} inputProps={{ maxLength: 3 }} required /></Grid></Grid>
      <Paper variant="outlined" sx={{ p: 2 }}><Stack direction={{ xs: 'column', sm: 'row' }} spacing={2} alignItems={{ sm: 'center' }}>{previewLogo}<Box sx={{ flex: 1 }}><Typography fontWeight={750}>{brand.logoFileName || 'No logo uploaded'}</Typography><Typography variant="body2" color="text.secondary">PNG or JPEG · maximum 1 MB · used on screens and PDF reports</Typography></Box><Stack direction="row" spacing={1}><Button component="label" variant="outlined" startIcon={<CloudUploadOutlined />}>Choose logo<input hidden type="file" accept="image/png,image/jpeg" onChange={event => void selectLogo(event.target.files?.[0])} /></Button>{brand.logoDataUrl && <Button color="error" startIcon={<DeleteOutline />} onClick={() => setBrand(current => ({ ...current, logoDataUrl: '', logoFileName: '' }))}>Remove</Button>}</Stack></Stack></Paper>
      <Divider /><Box><Typography variant="h5">Colours and sign-in</Typography><Typography color="text.secondary">The colours update application controls and the report accent treatment.</Typography></Box>
      ...
      <Divider /><Box><Typography variant="h5">Support and report identity</Typography><Typography color="text.secondary">These details help technicians and customers identify the report owner.</Typography></Box>
      ...
      <Grid size={{ xs: 12, xl: 5 }}><Stack spacing={3} sx={{ position: { xl: 'sticky' }, top: { xl: 88 } }}><Card><CardContent><Typography variant="overline" color="primary">Application preview</Typography><Paper variant="outlined" sx={{ mt: 1, p: 3, minHeight: 260, background: 'linear-gradient(135deg, #101a2d, #080f1d)' }}><Stack direction="row" spacing={2} alignItems="center">{previewLogo}<Box><Typography variant="caption" color="text.secondary">MSP PLATFORM</Typography><Typography variant="h5">{brand.name || 'CMDB Hub'}</Typography></Box></Stack><Divider sx={{ my: 3 }} /><Typography variant="h4">{brand.welcomeMessage || 'Asset intelligence for your organisation.'}</Typography><Typography color="text.secondary" sx={{ mt: 2 }}>{brand.supportEmail || brand.supportPhone || brand.supportUrl || 'Support details not configured'}</Typography></Paper></CardContent></Card>
        <Card><CardContent><Stack direction="row" spacing={1} alignItems="center"><InsertPhotoOutlined color="primary" /><Typography variant="overline" color="primary">Report preview</Typography></Stack><Paper variant="outlined" sx={{ mt: 1, p: 2.5, bgcolor: '#fff', color: '#172033', minHeight: 260 }}><Stack direction="row" alignItems="center" spacing={2} sx={{ bgcolor: '#0b1526', color: '#fff', p: 2 }}>{previewLogo}<Box sx={{ flex: 1 }}><Typography variant="h6" color="inherit">Change control package</Typography><Typography variant="caption" sx={{ color: '#d7e5ea' }}>{brand.name}</Typography></Box><Typography variant="caption" sx={{ color: '#d7e5ea' }}>CHG-2026-0001</Typography></Stack><Box sx={{ height: 5, bgcolor: brand.accent, mt: 2, borderRadius: 4 }} /><Typography fontWeight={800} sx={{ mt: 2 }}>Change summary</Typography><Typography variant="body2" sx={{ color: '#5e6b80' }}>Customer, schedule, risk and impacted configuration items</Typography><Divider sx={{ my: 3, borderColor: '#d8e1e8' }} /><Stack direction="row" justifyContent="space-between"><Typography variant="caption" sx={{ color: '#5e6b80' }}>{brand.reportFooter || `${brand.name} | Controlled change record`}</Typography><Typography variant="caption" sx={{ color: '#5e6b80' }}>{brand.confidentialityLabel || 'Internal use only'}</Typography></Stack></Paper></CardContent></Card></Stack></Grid></Grid>
  </RootGuard>;
}
...
export function DatabasePage() {
  ...
  return <RootGuard adminOnly><Title title="Database" /><PageHeading eyebrow="Settings" title="Database and recovery" copy="Review PostgreSQL readiness, initialise an empty database, and manage portable operational exports. PostgreSQL point-in-time recovery remains an infrastructure responsibility." action={status && <Chip label={status.available && !status.migrationsPending ? 'PostgreSQL ready' : status.mode} color={status.available && !status.migrationsPending ? 'success' : 'warning'} />} />
    {notice && <Alert severity={notice.severity} sx={{ mb: 2 }}>{notice.message}</Alert>}
    {!status ? <LoadingCard /> : <Grid container spacing={3}>
      <Grid size={{ xs: 12 }}><Card><CardContent><Stack direction={{ xs: 'column', lg: 'row' }} spacing={3} justifyContent="space-between"><Box><Typography variant="h5">Database readiness</Typography><Typography color="text.secondary">{status.database || settings.database || 'No database'} · {status.server || status.mode}</Typography></Box><Stack direction="row" spacing={1} flexWrap="wrap" useFlexGap><Chip label={status.available ? 'Reachable' : 'Unavailable'} color={status.available ? 'success' : 'error'} /><Chip label={`Schema ${status.schemaVersion || 'not installed'}`} color={status.migrationsPending ? 'warning' : 'success'} variant="outlined" /><Chip label={status.initialized ? 'Operational state seeded' : 'Awaiting seed'} color={status.initialized ? 'success' : 'warning'} variant="outlined" /><Chip label={status.databaseUser || 'No database user'} variant="outlined" /></Stack></Stack>{(status.error || status.diagnosticError) && <Alert severity="warning" sx={{ mt: 2 }}>{status.error || status.diagnosticError}</Alert>}</CardContent></Card></Grid>
      <Grid size={{ xs: 12, xl: 7 }}><Card><CardContent>{managed ? <Stack spacing={2}><Box><Typography variant="h5">Managed PostgreSQL connection</Typography><Typography color="text.secondary">Source: environment or Key Vault-backed application setting</Typography></Box><Alert severity="info">This connection is intentionally read-only here. Update the Azure Container App environment or Key Vault reference, then restart the service.</Alert><Grid container spacing={2}><Grid size={{ xs: 12, md: 8 }}><TextField fullWidth disabled label="Host" value={settings.host || ''} /></Grid><Grid size={{ xs: 12, md: 4 }}><TextField fullWidth disabled label="Port" value={settings.port || 5432} /></Grid><Grid size={{ xs: 12, md: 6 }}><TextField fullWidth disabled label="Database" value={settings.database || ''} /></Grid><Grid size={{ xs: 12, md: 6 }}><TextField fullWidth disabled label="Username" value={settings.username || ''} /></Grid></Grid><Button variant="outlined" onClick={() => void load()}>Refresh readiness</Button></Stack> : <Stack component="form" spacing={2} onSubmit={save}><Box><Typography variant="h5">PostgreSQL connection and initialisation</Typography><Typography color="text.secondary">Local setup only. Production credentials should be injected from Key Vault.</Typography></Box><Grid container spacing={2}><Grid size={{ xs: 12, md: 8 }}><TextField fullWidth label="Host" value={settings.host || ''} onChange={event => update('host', event.target.value)} required /></Grid><Grid size={{ xs: 12, md: 4 }}><TextField fullWidth label="Port" type="number" value={settings.port || 5432} onChange={event => update('port', Number(event.target.value))} required /></Grid><Grid size={{ xs: 12, md: 6 }}><TextField fullWidth label="Database" value={settings.database || ''} onChange={event => update('database', event.target.value)} required /></Grid><Grid size={{ xs: 12, md: 6 }}><TextField fullWidth label="Username" value={settings.username || ''} onChange={event => update('username', event.target.value)} required /></Grid><Grid size={{ xs: 12, md: 6 }}><TextField fullWidth label="Password" type="password" value={settings.password || ''} onChange={event => update('password', event.target.value)} /></Grid><Grid size={{ xs: 12, md: 6 }}><FormControl fullWidth><InputLabel>SSL mode</InputLabel><Select label="SSL mode" value={settings.sslmode || 'prefer'} onChange={event => update('sslmode', event.target.value)}><MenuItem value="prefer">Prefer SSL</MenuItem><MenuItem value="require">Require SSL</MenuItem><MenuItem value="verify-full">Verify full certificate</MenuItem><MenuItem value="disable">Disable SSL (local only)</MenuItem></Select></FormControl></Grid></Grid><FormControl fullWidth><InputLabel>Blank database seed</InputLabel><Select label="Blank database seed" value={settings.seedMode || 'current'} onChange={event => update('seedMode', event.target.value)}><MenuItem value="current">Migrate this workspace</MenuItem><MenuItem value="empty">Platform setup only</MenuItem><MenuItem value="demo">Demo customers and assets</MenuItem></Select></FormControl><Typography variant="caption" color="text.secondary">The seed choice is used only when the target database has no CMDB operational state.</Typography><Stack direction="row" spacing={1}><Button variant="outlined" type="button" onClick={test}>Test connection</Button><Button variant="contained" type="submit" startIcon={<StorageOutlined />}>Migrate and activate</Button></Stack></Stack>}</CardContent></Card></Grid>
      <Grid size={{ xs: 12, xl: 5 }}><Stack spacing={3}><Card><CardContent><Typography variant="h5">Portable export and import</Typography><Typography color="text.secondary" sx={{ mb: 2 }}>Moves operational CMDB data between installations. It is checksum-protected, but it is not a PostgreSQL backup or point-in-time restore.</Typography><Stack spacing={1.5}><Button variant="contained" startIcon={<BackupOutlined />} onClick={downloadBackup}>Download portable export</Button><Button color="warning" variant="outlined" startIcon={<RestoreOutlined />} onClick={() => setRestoreOpen(true)}>Validate and import export</Button>{backup && <Typography variant="caption" color="text.secondary">A versioned portable export was prepared during this session.</Typography>}</Stack></CardContent></Card><Card><CardContent><Typography variant="h5">PostgreSQL recovery</Typography><Typography color="text.secondary">Use Azure Database for PostgreSQL point-in-time restore in production, or scheduled <code>pg_dump</code>/<code>pg_restore</code> outside the web process. Retain and test those backups independently from portable exports.</Typography></CardContent></Card></Stack></Grid>
    </Grid>}
    <Dialog open={restoreOpen} onClose={() => setRestoreOpen(false)} maxWidth="sm" fullWidth><DialogTitle>Import portable CMDB export</DialogTitle><DialogContent><Alert severity="info" sx={{ mb: 2 }}>This adds or updates operational records. It does not delete records absent from the file and does not replace PostgreSQL recovery.</Alert><Button component="label" variant="outlined" fullWidth>{restoreFileName || 'Choose portable CMDB export'}<input hidden type="file" accept="application/json,.json" onChange={event => void chooseRestoreFile(event.target.files?.[0])} /></Button>{restorePreview && <Paper variant="outlined" sx={{ mt: 2, p: 2 }}><Typography fontWeight={800}>Validated version {restorePreview.version}</Typography><Typography variant="body2" color="text.secondary">{restorePreview.companies} customers · {restorePreview.users} users · {restorePreview.assets} CIs · {restorePreview.relationships} relationships · {restorePreview.changes} changes</Typography></Paper>}<FormControlLabel sx={{ mt: 2 }} control={<Checkbox checked={restoreConfirmed} onChange={(_, checked) => setRestoreConfirmed(checked)} />} label="I understand this is an import/merge and have downloaded a fresh export first." /></DialogContent><DialogActions><Button onClick={() => setRestoreOpen(false)}>Cancel</Button><Button color="warning" variant="contained" disabled={!restoreDocument || !restorePreview || !restoreConfirmed} onClick={restore}>Import operational data</Button></DialogActions></Dialog>
  </RootGuard>;
}
