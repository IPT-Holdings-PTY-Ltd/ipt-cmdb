...
export function BusinessSystemsPage() {
  ...
  return <Box>
    ...
    <Stack direction={{ xs: 'column', sm: 'row' }} justifyContent="space-between" alignItems={{ xs: 'stretch', sm: 'flex-end' }} spacing={2} sx={{ mb: 3 }}>
      <Box><Typography variant="overline" color="primary">Business service portfolio</Typography><Typography variant="h3">Business systems</Typography><Typography color="text.secondary">Connect systems people recognise to the technical CIs that keep them running.</Typography></Box>
    ...
    {loading ? <Stack alignItems="center" spacing={2} sx={{ py: 8 }}><CircularProgress /><Typography color="text.secondary">Loading the service portfolio…</Typography></Stack> : !systems.length ? <Alert severity="info" action={canEdit ? <Button color="inherit" onClick={() => setDraft(blankDraft())}>Add the first one</Button> : undefined}>No business systems are recorded for this customer yet.</Alert> : <Grid container spacing={2}>
      {systems.map(system => {
        const health = system.metadata?.operationalStatus || 'unknown';
        const dependencies = dependencyCount(system.id, relationships);
        return <Grid key={system.id} size={{ xs: 12, lg: 6 }}><Card variant="outlined" sx={{ height: '100%', borderTop: `3px solid ${system.metadata?.criticality === 'critical' ? '#ff7961' : '#50d5b9'}` }}>
          <CardContent>
            <Stack direction="row" justifyContent="space-between" spacing={2}><Stack direction="row" spacing={1.5}><BusinessCenterOutlined color="primary" /><Box><Typography variant="h6">{system.name}</Typography><Typography variant="body2" color="text.secondary">{system.metadata?.aliases || system.metadata?.department || 'Business service'}</Typography></Box></Stack><Chip size="small" color={health === 'healthy' ? 'success' : health === 'critical' || health === 'offline' ? 'error' : 'warning'} label={health} /></Stack>
            ...
            <Grid container spacing={1.5} sx={{ mt: 1 }}>
              <Grid size={{ xs: 6, sm: 3 }}><Typography variant="caption" color="text.secondary">Criticality</Typography><Typography variant="body2">{system.metadata?.criticality || 'medium'}</Typography></Grid>
              <Grid size={{ xs: 6, sm: 3 }}><Typography variant="caption" color="text.secondary">Dependencies</Typography><Typography variant="body2">{dependencies || 'None linked'}</Typography></Grid>
              <Grid size={{ xs: 6, sm: 3 }}><Typography variant="caption" color="text.secondary">RTO / RPO</Typography><Typography variant="body2">{system.metadata?.rtoHours || '?'}h / {system.metadata?.rpoHours || '?'}h</Typography></Grid>
              <Grid size={{ xs: 6, sm: 3 }}><Typography variant="caption" color="text.secondary">Users</Typography><Typography variant="body2">{system.metadata?.userPopulation || 'Not recorded'}</Typography></Grid>
            </Grid>
            <Box sx={{ mt: 2, p: 1.5, bgcolor: 'rgba(121,151,255,.08)', borderRadius: 1 }}><Typography variant="caption" color="text.secondary">Business owner / signoff</Typography><Typography variant="body2">{system.metadata?.businessOwner || 'Business owner missing'} · {system.metadata?.signoffRequired === 'no' ? 'Signoff not required' : system.metadata?.signoffDelegate || 'Owner signs off'}</Typography></Box>
          </CardContent>
